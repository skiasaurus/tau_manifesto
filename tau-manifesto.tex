...
\noindent There are many more examples,\footnote{See ``$\pi$ Is Wrong!'' for a list of other formulas involving $2\pi$. Or just start browsing Wikipedia math pages. :-)} and the lesson is clear: there's something seemingly inevitable about $2\pi$.
...
\begin{enumerate}
  ...
  \item \emph{$\tau$ resembles $\pi$.} \\ $\tau$ is typographically similar to $\pi$, thereby evoking the same notion of a circle constant, while still looking different enough to avoid confusion. Unfortunately, the number of ``legs'' isn't quite right: it would be poetic if we could write $\pi = 2\tau$, but it wasn't meant to be.
