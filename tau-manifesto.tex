...
There is an intimate relationship between circles and \emph{angles}, as shown in \hyperref[fig:angle-arclength]{Figure~}\ref{fig:angle-arclength}. The lines in the figure cut off different lengths of arc (or \emph{arclengths}) from each of two concentric circles with different radii, but the angle~$\theta$ (theta) is the same in each case. The task of angle measurement is to create a system that captures this arclength- and radius-invariance.
