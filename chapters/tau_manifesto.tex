...
We begin repairing the damage wrought by $\pi$ by first understanding the notorious number itself. The traditional definition for the circle constant sets $\pi$ (pi) equal to the ratio of a circle's circumference to its diameter (width):\footnote{The symbol $\equiv$ means ``is defined as''.}
\begin{equation}
\label{eq:pi}
...
e^{i\tau} = 1 + 0.
\end{equation}
Eq.~\eqref{eq:euler_tau_zero}, \emph{without} rearrangement, actually does relate the five most important numbers in mathematics: $0$, $1$, $e$, $i$, and $\tau$.
