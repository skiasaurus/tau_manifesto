...
constante du cercle est $\pi$\,; mais, comme le note le mathématicien
\href{https://www.math.utah.edu/~palais/}{Bob Palais} dans son superbe article
«\,$\pi$ Is Wrong!\,» \footnote{Palais, Robert. «\,$\pi$ Is Wrong!\,», \emph{The
...
«\,$\pi$ Is Wrong!\,». Il est disponible en ligne sur
\href{https://www.math.utah.edu/~palais/pi.html}{https://www.math.utah.edu/~palais/pi.html}.},
$\pi$ \emph{n'est pas correct}. Il est temps de régler les choses.
...
Ça devrait être évident que $\pi$ n'est pas «\,mauvais\,» dans le sens où il
...
L'évaluation de l'équation~\eqref{eq:eulers_formula} à $\theta = \tau$ donne
...
Écrite en termes de $\tau$, on voit que la forme «\,originale\,» de l'identité
...
dimensions arbitraires\,\footnote{Cette discussion est basée sur un excellent
...
suit\,\footnote{Les géomètres et les topologues utilisent des définitions
incompatibles d'hypersphères\,; cette discussion utilise les définitions des
...
«\,intérieur\,» comme un point\,\footnote{Cela a du sens, car un point n'a pas
...
\end{equation}
où la fonction gamma est donnée par l'équation~\eqref{eq:gamma}.
...
Plutôt que de simplement prendre ces formules à leur valeur nominale, on fait
voir si l'on peut les démêler pour mieux éclairer la question de $\pi$ vs
...
fonction factorielle à des arguments à valeurs réelles\,\footnote{En effet, la
...
lorsque $n$ est impair. En tenir compte donne l'expression suivante, qui est
...
On fait examiner l'équation~\eqref{eq:volume_mathworld} plus en détail. On
remarque tout d'abord que MathWorld utilise la
\emph{double factorielle}~$n!!$---mais, étrangement, il ne l'utilise que dans le
...
On fait maintenant examiner le cas pair dans
...
\end{cases}
\end{equation}
L'intégration de l'équation~\eqref{eq:surface_area_tau} par rapport à $r$ donne
alors
\begin{equation}
\label{eq:volume_tau}
...
(l'équation~\eqref{eq:surface_area_mathworld} et
l'équation~\eqref{eq:volume_mathworld}) en termes de $\pi$. Mais en fait une
simplification supplémentaire est possible, en utilisant la mesure d'un angle
droit\,\footnote{Ce changement de notation et d'analyse générale était suggéré
par Jeff Cornell.}~:
\begin{equation}
\label{eq:lambda}
\lambda = \frac{\tau}{4}.
...
l'équation~\eqref{eq:tau_lambda} donne alors
\[
\begin{split}
...
chacun des quatre quadrants\,; en trois dimensions, ce sont les secteurs de la
sphère dans chaque octant\,; et ainsi de suite dans des dimensions supérieures.
...
n'implique pas d'intégrales compliquées---juste les fonctions légèrement
...
les dimensions\,\footnote{L'article
«\,\href{http://www2.math.uconn.edu/~mariano/research/MathClubsp14\%20.pdf}{The
volume of the unit ball in \emph{n} dimensions}\,» («\,Le volume de la balle
unité en $n$ dimensions\,») de Phanuel A. Mariano contient une dérivation
alternative de ces récurrences importantes.}. En particulier, on fait diviser le
...
divisant par $n$ et en multipliant par la «\,constante de récurrence\,»
$2^2\lambda$.
...
voit que ce n'est autre que $\tau$\,! En effet, une
...
\end{split}
\end{equation}
ce qui montre ainsi que l'identification de $\tau$ comme «\,constante de
récurrence\,» n'est pas un hasard---la constante de récurrence et la constante
...
(la
figure~\ref{fig:Nspheres})\,\footnote{\href{https://sites.google.com/site/taubeforeitwascool/}{Tau
Before It Was Cool} écrit en fait la récurrence en termes de $2\pi$\,; la
...
puis on utilisera la définition de $\pi$ (l'équation~\eqref{eq:pi}) pour en
définir une troisième, révéilant ainsi exactement ce qui ne va pas avec $\pi$.

Tout d'abord, on définira une famille de «\,constantes de superficie\,» $\tau_n$
...
\end{equation}
Deuxièmement, on définira une famille de «\,constantes de volume\,» $\sigma_n$
...
On fait faire quelques observations sur ces deux familles de constantes. La
...
$n$-sphère complète. En particulier, en écrivant $s_n(r)$ comme la «\,longueur
d'arc\,» à $n$ dimensions égale à une fraction~$f$ de la superficie
totale~$S_n(r)$, on a
\[
...
d'angle radian, et on voit que $\tau_n$ est la généralisation d'«\,un tour\,» à
...
section~\ref{sec:volume_of_a_hypersphere} que $\tau_2$ est aussi la «\,constante
de récurrence\,» pour les superficies et les volumes des $n$-sphères.
...
Alors, à quelle famille de constantes appartient naturellement $\pi$\,? On fait
réécrire l'équation~\eqref{eq:pi} en termes plus appropriés pour la
...
\begin{itemize}
  ...
  \item \textbf{Êtes-vous sérieux\,?} \\ Bien sûr. Je veux dire, je m'amuse avec
  ...
  inquiet pour eux\,; ils peuvent prendre soin d'eux-mêmes. Ce sont les
  ...
  que je veux dire\,? C'est de la folie---folie pure.

  \item \textbf{Comment passer de $\pi$ à $\tau$\,?} \\ La prochaine fois que
  vous écrivez quelque chose qui utilisa la constante du cercle, dites
  simplement «\,Par commodité, on définit $\tau=2\pi$\,», puis procédez comme
  d'habitude. (Bien sûr, cela pourrait simplement conduire à la question~:
  «\,Pourquoi voudrait-on faire cela\,?\,» Et j'admets que ce serait bien
  ...
  \item \textbf{N'est-il pas trop tard pour passer\,? Ne faudrait-il pas
  réécrire tous les manuels et articles mathématiques\,?} \\ Non sur les deux
  ...
  versa---maudissant ainsi les étudiants débutants en physique avec des signes
  négatifs déroutants depuis\,\footnote{On ne pouvait pas déterminer le signe
  ...
  de manière incrémentale\,; contrairement à une redéfinition, cela ne doit pas
  nécessairement se produire d'un seul coup.

  \item \textbf{L'utilisation de $\tau$ ne déroutera-t-elle pas les gens, en
  particulier les étudiants\,?} \\  Si vous êtes assez intelligent pour
  comprendre la mesure d'angle radian, vous êtes assez intelligent pour
  comprendre $\tau$---et pourquoi $\tau$ est en fait \emph{moins} déroutant que
  $\pi$. De plus, il n'y a rien de intrinsèquement déroutant dans l'énoncé
  «\,Soit $\tau=2\pi$\,»\,; compris étroitement, c'est juste une simple
  ...
  \item \textbf{Est-ce que cela importe vraiment\,?} \\ Bien sûr, cela importe.
  ...
  chiffres de la moitié de la vraie constante du cercle\,?) Puisque la constante
  ...
  corrigé le problème quand il en avait l'occasion\,; contrairement à Archimède,
  ...
  différents}\,! Quel dommage qu'il n'ait pas standardisé le choix le plus
  pratique.

  \item \textbf{Pourquoi êtes-vous intéressé par ce sujet\,?} \\ D'abord, en
  ...
  \item \textbf{Êtes-vous, genre, un fou\,?} \\ Ce n'est vraiment pas votre
  ...
  \item \textbf{Mais qu'en est-il des jeux de mots\,?} \\ Nous arrivons
  maintenant à la dernière objection. Je sais, je sais,
  «\,$\pi$ in the sky\,»\,\footnote{N.\ du T.~: Ce jeu de mots vient du fait
  ...
  ce qui signifie «\,tarte\,»\,;
  \href{https://en.wiktionary.org/wiki/pie_in_the_sky}{\emph{pie in the sky}}
  est un idiome anglais qui signifie «\,une notion fantaisiste\,».} est
  tellement très ingénieux. Et pourtant, $\tau$ lui-même regorge de
  possibilités. Le $\tau$isme\,\footnote{N.\ du T.~: En anglais, la
  ...
  de $\tau$---la moitié de $\tau$, pour être exact. L'identité $e^{i\tau} = 1$
  dit~: «\,\emph{Soyez un avec le $\tau$.}\,» Et même si l'observation selon
  laquelle «\,\emph{une rotation d'un tour vaut~1}\,» peut sembler une
...
profondes. De plus, il est livré avec un diagramme vraiment cool (la
figure~\ref{fig:tauism}). On voit sur la figure~\ref{fig:tauism} un mouvement à
travers le \emph{yang} («\,clair, blanc, montant\,») jusqu'à $\tau/2$ et un
retour à travers le \emph{yin} («\,sombre, noir, descendant\,») à
$\tau$\,\footnote{Les interprétations du yin et du yang citées ici sont tirées
...
% Si jamais on s'entend dire des choses comme «\, Parfois $\pi$ est le meilleur choix, et parfois c'est $2\pi$\,», on devrait s'arrêter et se souvenir des paroles de \href{http://vihart.com/}{Vi Hart} dans sa magnifique \href{https://www.youtube.com/watch?v=jG7vhMMXagQ}{vidéo sur $\tau$}~: «\,Non\,! Vous cherchez des excuses pour $\pi$.\,» Il est temps d'arrêter de chercher des excuses.
...
concerne les mathématiques\,\footnote{Puisque 6 et 28 sont les deux premiers
...
deux fois plus de \og pi(e)\,»\,\footnote{N. de T.~: Un autre jeu de mots\,;
...
Palais} d'avoir écrit «\,$\pi$ Is Wrong!\,» Je ne me souviens pas à quel point
mes soupçons sur $\pi$ étaient profonds avant de rencontrer cet article, mais
«\,$\pi$ Is Wrong!\,» certainement ouvert mes yeux, et chaque section du
...
incroyables vidéos inspirées de $\tau$, ainsi que Don «\,Blue\,» McConnell et
...
inspirée par une excellente suggestion de Timothy «\,Patashu\,» Stiles.
...
lancement\,; entre autres choses, si jamais vous avez besoin de quelqu'un pour
vous dire que «\,pratiquement toute de la section 5 [maintenant supprimée] est
de la merde totale\,», Wyatt est l'homme de la situation.
